...
const Info = () => {
    ...
    const isMatt = updatedDataStore((state) => state.isMatt)
    ...
    //----------------------------------------------------------------------------------------------------------------------------------
    function onTurnMessage() {
        console.log(isMatt)
        let msg = IsonTurn() === playerColor ? `you are on turn with ${playerColor === 1 ? 'White' : 'Black'}` : 'Wait' ;
        let msgOnMatt = IsonTurn() !== playerColor ? `You win ` : 'You Lose'
        return isMatt ? msgOnMatt : msg
    }

    return (
        <div className="gameInfo">
            <button onClick={() => exitGame()}>Exit</button>
            <div className={IsonTurn() === playerColor && isMatt === false ? 'onTurn':'notOnTurn'}>
                <p>{onTurnMessage()} </p>
            </div>
